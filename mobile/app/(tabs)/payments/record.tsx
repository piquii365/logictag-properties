          ...
          <>
            {unit ? (
              <Card className="flex-row items-center mb-5">
                <Avatar initials={(unit.tenant?.name ?? "T").slice(0, 2).toUpperCase()} tint="#F96B1F" />
                <View className="ml-3">
              ...
              <>
                <Text className="text-[13px] text-[#6B7280] mb-1.5">Tenant</Text>
                <Pressable
                ...
                >
                  <Text className="text-[15px] text-[#0F2C4A]">
                    {selectedTenant ? `${selectedTenant.firstName} ${selectedTenant.lastName}` : "Select a tenant"}
                  </Text>
                  <Ionicons name={pickTenant ? "chevron-up" : "chevron-down"} size={18} color="#9CA3AF" />
                </Pressable>
                {pickTenant ? (
                  <View className="bg-white border border-[#E5E9F0] rounded-xl -mt-2 mb-4 overflow-hidden">
                    {(tenants.data ?? []).length === 0 ? (
                      <Text className="text-[13px] text-[#6B7280] px-4 py-3">No tenants on record yet.</Text>
                    ) : (
                      (tenants.data ?? []).map((t) => (
                        <Pressable
                          key={t.id}
            ...
            <Text className="text-[13px] text-[#6B7280] mb-1.5">Payment Method</Text>
            <Pressable
            ...
            >
              ...
              <Ionicons name={pickMethod ? "chevron-up" : "chevron-down"} size={18} color="#9CA3AF" />
            </Pressable>
            {pickMethod ? (
              <View className="bg-white border border-[#E5E9F0] rounded-xl -mt-2 mb-4 overflow-hidden">
                {METHODS.map((m) => (
                  <Pressable
                    ...
                    className="px-4 py-3 active:bg-[#F8FAFC]"
                  >
                    <Text className="text-[14px] text-[#0F2C4A]">{m.label}</Text>
                  </Pressable>
                ))}
              </View>
            ) : null}
